...
def quizQuestions():
	if len(user) == 0:
		...
		if user[1] == "ADMIN":
			...
			with open("assets/question.json", 'r+') as f:
...
def createAccount():
	...
	with open('assets/user_account.json', 'r+') as user_account: #The r+ mode is used to open a file for both reading and writing
		users = json.load(user_account)
		if username in users.keys():
			print("An account of this Username already exists.\nPlease enter the login panel.")
		else:
			users[username] = [password, "PLAYER"]
			user_account.seek(0) # seek() function is used to change the position of the File Handle to a given specific position. 0 = beginnning
			json.dump(users, user_account) #dumps convert python objects of the all types such int,float,list,tuple,dictionary into JSON strings:
			user_account.truncate() # Python file method truncate() truncates the file's size.
...
def loginAccount():
	...
	with open('assets/user_accounts.json', 'r') as user_account:
		users = json.load(user_account)
...
def about():
	print('''\n==========ABOUT US==========
This project has been created by Sanskriti Soni.''')
